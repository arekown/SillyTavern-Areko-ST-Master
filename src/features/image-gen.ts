...
import { renderTemplate } from '../core/prompts';
...
export function setCharImage(name: string, url: string): void { commit((m) => { m[name] = url; }); }
export function deleteCharImage(name: string): void { commit((m) => { delete m[name]; }); }
...
function flattenToText(v: any): string {
  if (v == null) return '';
  if (typeof v === 'string') return v.trim();
  if (typeof v === 'number' || typeof v === 'boolean') return String(v);
  if (Array.isArray(v)) return v.map(flattenToText).filter(Boolean).join(', ');
  if (typeof v === 'object') return Object.values(v).map(flattenToText).filter(Boolean).join(', ');
  return '';
}
...
export function buildImagePrompt(name: string): string {
  ...
  if (found && s.imageGen.sourceFieldId) {
    ...
    if (path) details = flattenToText(readPath(found.entry, path));
  }
  const text = renderTemplate(s.imagePrompt || '', { name, details });
...
export async function generateCharImage(name: string): Promise<void> {
  ...
  if (!prompt) throw new Error('Kein Bild-Prompt (Quellfeld leer?).');
