import { ExtensionSettings, Preset } from '../config/types';
...
export function resetPreset(s: ExtensionSettings, key: string): void {
  const p = s.presets[key];
  if (p) {
    const def = makeDefaultPreset(p.name);
...
export function importPreset(s: ExtensionSettings, json: string): string {
  ...
  s.presets[key] = {
    ...
    characterRules: parsed.characterRules ?? { loopOverCharacters: false, excludedCharacters: [] },
  };
  s.activePreset = key;
  return key;
}